...
<!-- gitnexus:end -->
utilise tout les skills necesaires de claude pour menr a bien la mission 
si tu le fais pas bien en tant que expert je vais le donner a codex 
# CLAUDE.md — Instructions Claude Code · admin ta-quota
...
**Déclencheur :** nouvelle feature, refactor risqué, migration. Claude pose des questions jusqu'à épuiser tous les cas.
```
...
**Déclencheur :** construire une app avec l'Anthropic SDK, Agent SDK, appels API Claude.
```
Skill local : claude-api
```

### Product Self-Knowledge (Anthropic)
**Déclencheur :** question sur les modèles Claude, pricing, limites de tokens, SDK.
```
...
### Update Config
**Déclencheur :** modifier `settings.json`, hooks Claude Code, permissions.
```
...
### Everything Claude Code (Conventions ECC)
**Déclencheur :** configurer Claude Code correctement, hooks, rules, conventions du projet.
```
Skill local : everything-claude-code
```
...
**Déclencheur :** faire collaborer Claude avec des agents OpenAI / Codex.
```
...
```
github.com/ComposioHQ/awesome-claude-skills/api-docs-generator
```
...
### Keybindings Help
**Déclencheur :** "quels sont les raccourcis ?", aide sur les keybindings Claude Code.
```
...
Ces skills sont installés dans l'environnement Claude Code mais ne sont pas dans `/mnt/skills/`.
Les appeler par leur nom dans la session.

```
Core          : update-config, keybindings-help, simplify, loop, schedule, claude-api
...
Ecosystem     : top-50-ai-tools, everything-claude-code
